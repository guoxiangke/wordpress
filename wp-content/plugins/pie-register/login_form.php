...
if ( isset($_GET['action']) && ('rp' == $_GET['action'] || 'resetpass' == $_GET['action']) && ($newpasspageLock == 0) ){
	...
	$form_data .= '
		...
		$form_data .= '<p class="comment-form-social-connect">
							<label>Or Login with</label>
						</p><div class="social_connect_form">';
		if( $facebook_enabled )
			$form_data .= apply_filters('social_connect_login_facebook','<a href="javascript:void(0);" title="Facebook" class="social_connect_login_facebook"><img alt="Facebook" src="'.$images_url.'facebook_32.png" /></a>');

		if( $facebook_enabled )
			$form_data .= apply_filters('social_connect_login_twitter','<a href="javascript:void(0);" title="Twitter" class="social_connect_login_twitter"><img alt="Twitter" src="'.$images_url.'twitter_32.png" /></a>');
		$registration_from_fields .= '</div>';

		// add other links
			$form_data .= '<p class="nav">
				<a class="simplemodal-register-" href="http://everydayparenting.proferochina.com/pie-register-registration">Register</a> | <a class="simplemodal-forgotpw-" href="http://everydayparenting.proferochina.com/pie-register-forgot-password" title="Password Lost and Found">Lost your password?</a>
			</p>';
		/**
		 * Dale code end.
		 * Add social connect in login form. 
